...
interface ProductStore {
  ...
  fetchProductsByCategory: (category: string, limit?: number) => Promise<Product[]>
  fetchFlashSaleProducts: (limit?: number) => Promise<Product[]>
  fetchBestsellers: (limit?: number) => Promise<Product[]>

  // Admin operations
  createProduct: (productData: CreateProductData, images: File[]) => Promise<Product | null>
...
export const useProductStore = create<ProductStore>()((set, get) => ({
  ...
  fetchFlashSaleProducts: async (limit) => {
    ...
    try {
      ...
      set({ error: error.message || "Failed to fetch flash sale products", loading: false })
      return []
    }
  },
  ...
  fetchBestsellers: async (limit) => {
    ...
    try {
      ...
      set({ error: error.message || "Failed to fetch bestsellers", loading: false })
      return []
    }
  },
