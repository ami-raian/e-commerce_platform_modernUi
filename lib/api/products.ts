 ...
 */
export async function getProductsAxios(filters?: ProductFilters): Promise<ProductsResponse> {
 ...
 */
export async function getFlashSaleProductsAxios(limit?: number): Promise<Product[]> {
  const response = await apiClient.get<ApiResponse<{ products: Product[] }>>('/products/flash-sale', {
    params: { limit },
  })
  return response.data.data!.products
}
...
export async function getBestsellersAxios(limit?: number): Promise<Product[]> {
  const response = await apiClient.get<ApiResponse<{ products: Product[] }>>('/products/bestsellers', {
    params: { limit },
  })
  return response.data.data!.products
}
 ...
 */
export async function checkAvailabilityAxios(id: string, quantity: number): Promise<boolean> {
 ...
 */
export async function addReviewAxios(productId: string, reviewData: AddReviewData): Promise<Product> {
 ...
 */
export async function createProductAxios(productData: CreateProductData): Promise<Product> {
 ...
 */
export async function updateProductImagesAxios(id: string, images: File[]): Promise<Product> {
  const formData = new FormData()
 ...
 */
export async function updateStockAxios(id: string, quantity: number): Promise<Product> {
  const response = await apiClient.patch<ApiResponse<{ product: Product }>>(
    `/products/${id}/stock`,
    { quantity }
  )
...
export async function getProducts(
  ...
  const response = await apiFetch<ApiResponse<ProductsResponse>>(`/products${queryString}`, {
    cache: 'force-cache',
 ...
 */
export async function getProductById(id: string, revalidate?: number): Promise<Product> {
 ...
 */
export async function getFlashSaleProducts(limit?: number, revalidate?: number): Promise<Product[]> {
  const queryString = limit ? `?limit=${limit}` : ''
  const response = await apiFetch<ApiResponse<{ products: Product[] }>>(
 ...
 */
export async function getBestsellers(limit?: number, revalidate?: number): Promise<Product[]> {
  const queryString = limit ? `?limit=${limit}` : ''
  const response = await apiFetch<ApiResponse<{ products: Product[] }>>(
